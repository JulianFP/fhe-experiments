...
On a more architectural level we would also like to point out that any given experiment only ran on one machine (either the cluster or the PC).
...
Regarding the server-side \ac{fhe} processing the experiments environment also could be improved since the cluster was shared by other users which impacted the runtimes, and the PC is not representative of server and cloud machines that \ac{fhe} processing would run on.
