...
Because of this even modern bootstrappable \ac{fhe} schemes feature very cheap decryption, especially compared to traditional encryption schemes like \acused{rsa}\ac{rsa} \cite{gentry_fully_2009}.
...
Most other schemes however (with the \acused{ntru}\ac{ntru} branch being a notable exception) are based on some kind of lattice problem.
...
\subsection{The \acf{lwe} Problem}\label{LWE}
...
\subsection{\acf{tfhe}}\label{TFHE}
...
\subsubsection{\ac{lwe} ciphertexts}
...
\subsubsection{\ac{rlwe} ciphertext}
...
\subsubsection{\ac{rgsw} ciphertext}
...
\subsubsection{Additional \ac{tfhe} building blocks}\label{TFHE_building_blocks}
...
The \textbf{\ac{cmux}} is a common binary gate with three inputs $d_0, d_1, b$ and one output $d_b$.
...
\subsection{\acf{cart}}\label{CART}
...
\subsubsection{One-Hot Encoding and \acf{bow}}
...
\begin{verbatim}
    I like using a ML model.
I feel like I am using black magic.
\end{verbatim}
