...
The topic of this thesis falls into an overlap between the fields of cryptography and \ac{ml}, and as such we will first introduce some concepts from both fields in this chapter.
...
Our secret key $p$ is a sufficiently large, odd integer.

We will now encrypt our $m$ into a ciphertext $c$:
...
So in other words a challenger generates the required parameters, computes $x_1$ to $x_n$, and provides an adversary with only these computed values who then has to find $p$.
To draw the parallel to our encryption in \cref{eq:dijk_encryption}, the $x_i$ are many different ciphertexts, and the $r_i$ are results of $2r + m$.

While this looks very similar to the well-known \ac{gcd} problem, the key difference here is the random noise $r_i$ which make the results only approximates of common divisors of $p$.
Consequently this variation is called \acf{agcd}.
While the \ac{gcd} problem can be efficiently solved using Euclid's algorithm, \ac{agcd} with correctly chosen parameters on the other hand is hard to solve.
...
We can also see some deficits of this simple homomorphic encryption scheme here: The noise $r$ grows linearly during addition (see the $r_1 + r_2$ term in \cref{eq:dijk_hom_add}), and quadratically during multiplication ($r_1r_2$ in \cref{eq:dijk_hom_mult}).
Since these homomorphic evaluations only stay correct while the noise is sufficiently smaller than $p$, we can only homomorphically evaluate a limited amount of additions and multiplications before decryption would output an incorrect result.
This is why Gentry called these kind of schemes \emph{somewhat} homomorphic encryption schemes \cite{gentry_fully_2009}, while also providing us with a general algorithm to turn somewhat homomorphic schemes into fully homomorphic ones which we will have a look at in \cref{Bootstrapping}.

A second deficiency of this scheme is that the ciphertext grows in size during homomorphic evaluation, e.g. during multiplication the 'key-part' of the ciphertext grows quadratically (the $p^2 q_1 q_2$ term in \cref{eq:dijk_hom_mult}).
In other words this scheme does not compactly evaluate circuits, as defined by Gentry in definition 2.1.2 and 2.1.3 \cite{gentry_fully_2009}.
\emph{Compact Homomorphic Encryption} requires the decryption complexity to not be dependent on the circuit depth which in turn requires the ciphertext and keys (which are the inputs of the decryption algorithm) to not grow with the depth of the circuit.

Finally our scheme in its current form also lacks circuit privacy, another attribute introduced by Gentry in \cite{gentry_fully_2009}.
A regular encryption scheme guarantees that if Alice encrypts a plaintext and sends it to Bob, Bob is not able to infer anything about the contents of the plaintext from the ciphertext.
With homomorphic encryption an additional desired attribute is that, if Bob is performing some operations on Alice's ciphertext, i.e. applying a circuit to it, and then sends the resulting ciphertext back to Alice, that Alice is then not able to infer anything about the nature of Bob's circuit.
She should only be able to decrypt the result of Bob's algorithm, not learn anything about it.
This is called \emph{(Statistical) Circuit Private Homomorphic Encryption} since it essentially mandates that the probability distribution of a resulting ciphertext should be the same regardless of whether a circuit C was homomorphically applied to it, or C was applied directly to the plaintext and its result was encrypted afterwards.
While our scheme currently does not fulfill this attribute, Gentry also points out that most schemes can become circuit private by adding encryptions of '0' to the result many times, randomizing the ciphertext.
Also many schemes only fulfill a relaxed version of circuit privacy where the resulting ciphertext is allowed to reflect the depth of the circuit that got applied to it, but no details of its exact nature \cite{gentry_fully_2009}.

\begin{remark}
    While we constructed this scheme as a symmetric encryption scheme, in homomorphic encryption any symmetric scheme can be converted into an asymmetric scheme by using many encryptions of '0' as the public key, e.g. in our scheme integers $x_i = q_i p + 2r_i$.
    Then anybody can create a valid ciphertext for their bit $m$ by adding one of the encrypted zeros to it.
    For randomization purposes the encrypting party should even add multiple encrypted zeros to the bit \cite{van_dijk_fully_2010}.
\end{remark}

While this simplified version of the \ac{dghv} scheme is not representative of modern \ac{fhe} schemes, it helps to understand the general goals and struggles of \ac{fhe}.
...
Due to limitations like this, every homomorphic encryption scheme prior to 2009 was not able to evaluate a computation of arbitrary depth on a ciphertext.
Gentry then introduced a new quite ingenious technique called \emph{Bootstrapping} in his PhD thesis which enabled him to build the first Fully Homomorphic Encryption scheme \cite{gentry_fully_2009}.

The basic principle behind the bootstrapping approach is to build a somewhat homomorphic encryption scheme that is able to homomorphically evaluate its own decryption circuit.
It turns out that this decreases the noise of the given ciphertext.
The general strategy becomes to apply computations on the ciphertext while keeping track of its noise.
Ones the noise reaches a certain threshold we apply bootstrapping to get a new, equivalent ciphertext with lower noise, on which we can then continue our computations until the noise reaches this threshold again and so on.
This means that as long as we can build a somewhat homomorphic encryption scheme that can evaluate computations of a depth larger than the depth of its decryption circuit plus some additional operations, we get a fully homomorphic encryption scheme that can evaluate circuits of any depth on ciphertexts.
Such schemes are called \emph{bootstrappable}.

We will now look at bootstrapping in more detail and explain on a high level why it works:
We already know that with a homomorphic encryption scheme we can evaluate a function $f$ on some ciphertext $c \xleftarrow{\$} \mathrm{Encrypt}_{sk_1}(m)$ and get the encryption of the result of the function f $\mathrm{Encrypt}_{sk_1}(f(m))$, with $sk_1$ being a secret key.
For bootstrapping, we now introduce a second secret key $sk_2$, and define our function as $f(x) = \mathrm{Decrypt}_x(c)$.
Please note that our function is the decryption algorithm, and that the input of $f$ is not the ciphertext that should be decypted, but the key that should be used to decrypt the ciphertext.
...
This is called circular security and severely hardens the schemes security analysis.
...
To achieve this Gentry not only tried to increase the maximum circuit depth the encryption scheme could evaluate, but also to decrease the depth of the schemes decryption circuit.
...
Gentry achieved this by not only choosing an encryption scheme that inherently has cheap decryption, but also by squashing the decryption circuit even further at the cost of a more expensive encryption.
As a result even modern bootstrappable \ac{fhe} schemes feature very cheap decryption, especially compared to traditional encryption schemes like \acuse{rsa}\ac{rsa} \cite{gentry_fully_2009}.

\subsection{The Path to Modern \acs{fhe} Schemes}\label{path_to_modern_schemes}
...
Most of today's \ac{fhe} schemes can be classified into one of these two main branches:
...
It can evaluate arbitrary circuits with arbitrary depths, and the party who encrypts the inputs does not have to know anything about these circuits at encryption time, making these schemes very flexible \cite{chillotti_tfhe_2021}.
...
While these limitations make these schemes less flexible, they mostly matter if in a specific application the circuit depth is not known beforehand by the party encrypting the inputs, e.g. because the circuit depth is dynamic and changes regularly.
...
In \cref{DGHV} we built a simple homomorphic encryption scheme based on the \ac{agcd} problem (the \ac{dghv} scheme).
...
Its improved version \ac{bfv} \cite{fan_somewhat_2012} which came out shortly after, is also considered to be the first truly practical \ac{fhe} scheme \cite{noauthor_continuing_nodate}.
The \ac{ckks} scheme was a further advancement on this development branch and currently forms, together with \ac{tfhe}, the basis of most modern \ac{fhe} developments and research \cite{cheon_homomorphic_2017}.
...
However with sufficiently large errors $s$ becomes unrecoverable using just Gaussian elimination, making this a hard problem to solve.
...
This however requires trying many possible $s$ by brute-force, which is why the runtime of this algorithm is in $2^{O(n)}$.
There are no algorithms that improve this runtime significantly, making this problem NP-hard \cite{regev_lattices_2005}.

The \ac{lwe} problem can be reduced to lattice problems such as the \ac{cvp} which is why \ac{lwe} cryptosystems are also considered to be lattice-based.
...
Given a basis for a lattice $B$ and a vector $t \in \mathbb{R}^n$, the \ac{cvp} problem is to find a vector $v \in L(B)$ such that this vector is the closest vector to $t$ in the lattice.
...
While solving this looks quite trivial on the picture, please note that the complexity was $2^{O(n)}$, with $n$ being the dimensionality that is only set to $2$ in this image.
Also this is the trivial basis that already includes the shortest vectors of the lattice which makes the \ac{cvp} much easier.
...
We will not explain all these operations exhaustively but just give a broad overview to give some understanding of why mapping arbitrary computations to the most efficient set of \ac{fhe} operations is a challenging task.

\subsubsection{\acs{lwe} ciphertexts}

We will start off with \ac{lwe} ciphertexts: These use the \ac{lwe} problem as explained in \cref{LWE}, and encode the message $m$ from a message space $\mathcal{M}$ in what before was the error of the \ac{lwe} equations.
...
Decryption then just becomes subtracting the inner product between $a$ and $s$ from $b$:
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.45\textwidth]{../figures/lwe_message_encoding.pdf}
    \end{center}
...
Essentially, in the image, each possible encoded message without any error ($\Delta m$) corresponds to one of the blue lines while the red lines represent the thresholds inside which the error can shift the encoded message without changing its meaning.
If $|e|$ were to grow to $6$ or larger, then decoding would not yield our original message anymore \cite{chillotti_tfhe_2020} \cite{chillotti_tfhe_2021}.

We can perform both addition (\cref{eq:LWE_addition}) and multiplication with a constant factor $\gamma \in \mathcal{M}$ (\cref{eq:LWE_multiplication}) directly on \ac{lwe} ciphertexts.
...
This ciphertext works equivalently to the \ac{lwe} ciphertext, with the only difference being that instead of having integers modulo $p$, we now work with polynomials modulo $X^N + 1$.
Hence the message, secret, our factors, and the error become polynomials: $M(X), S(X), A(X), B(X), E(X)$, encoding $N$ coefficients.
...
Decoding also mostly stays the same, with the only difference being that the process has to be repeated $N$ times for every coefficient of our polynomial (with $\Delta M_0 + E_0$ for the first coefficient, $\Delta M_1 + E_1$ for the second one, and so on).

On this ciphertext we can also perform addition and multiplication by a constant, except that this time our constant is a constant polynomial $\Gamma(X)$.
...
Because of its complex structure we will not explain it in detail and just give a high level overview.
This 3-dim matrix is constructed as $l$ different $2\times2$ matrices, where the $j$th $2\times2$ matrix consists of two encryptions of our message: $(A_j(X), B_j(X))$ (the first row), and $(A_j^*(X), B_J^*(X))$ (the second row).
...
\ac{rgsw} allows addition and multiplication with a constant as well which work the same way as for \ac{rlwe}, but applied for every single \ac{rlwe} encryption in our matrix.
...
This is also why \ac{cmux} is so important in \ac{tfhe}, it is being used to implement conditional assignment, i.e. an if-condition for variable assignment.
It is implemented using three other operations in the following way (with $\odot$ being the external product, and $-,+$ being addition between \ac{rlwe} ciphertexts):
...
This is achieved easily by multiplying the polynomial with $X^{-p}$, and because of the $\mathrm{mod}\ X^N + 1$ this will automatically rotate the polynomial (plus changing the signs of the coefficients that are 'falling of' at the left):
...
This is implemented by having $p$ be encrypted bit-by-bit, i.e. we have encryptions of $p_0$, $p_1$,...,$p_k$ where each of these $p_i$ is one bit of $p$.
...
\section{\acl{ml} Concepts and Model Types}

We will now switch over to introduce some concepts of \ac{ml} that we specifically depended on in our later experiments.
...
The sigmoid function essentially just maps its input to the range $[0, 1]$, so that the models output can be interpreted as a probability for the Bernoulli distribution \cite{murphy_machine_2013}:
\begin{align}
...
This is a linear loss that for each sample, adds the computed gradient to the model weights if the classification was incorrect, and does not change the weights if it was correct.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \caption{The partitioned training data with each region colored in its label ($y_1$ in red, $y_2$ in blue, $y_3$ in green)}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
...
If we want to draw a hyper-cube around our vector $x$ that contains $1\%$ of the data, its expected edge length has to be $0.01^{\frac{1}{D}}$, with $D$ being our dimensionality.
...
\subsubsection{\acl{tfidf}}\label{TFIDF}
...
Other more complex tasks however require our model to gain an intricate understanding of the semantics of any given word and thus its positional relation to other words in a sentence.
...
For example if two words can be interchanged in most sentences without drastically changing the sentences meaning or correctness, we would expect the embeddings of these two words to have a very high cosine similarity.
...
Semantic embeddings are generated by a model specifically trained for this task, like a Word2Vec model or, more commonly nowadays and with much improved results, transformer based models like \ac{bert}.
...
\end{verbatim}
the same token 'Paris' now functions as a persons first name. But not just synonyms are problematic:
\begin{verbatim}
