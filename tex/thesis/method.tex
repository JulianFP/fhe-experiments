...
In this chapter we will describe the experiment setup used to measure various trade-offs of homomorphic encryption in different \ac{ml} scenarios.
...
The experiments were implemented purely in Python, the de facto standard language in the field of \ac{ml}.
Being able to stick with a high-level language as well as other industry standard tools like \emph{scikit-learn} and \emph{PyTorch} is not self-evident when implementing \ac{fhe} programs and is only possible due to the ease-of-use of the \ac{fhe} framework \emph{concrete-ml} (see \cref{concrete-ml}).
...
The python package also uses \emph{click} \cite{pallets_welcome_nodate} to add a convenient \acs{cli} to be able to adjust parameters like execution repetitions, which experiment-dataset combinations should be executed, or which graphs should be generated.

\section{The Examined \acs{fhe} Framework: concrete-ml}\label{concrete-ml}

We chose the \emph{concrete-ml} framework to implement a variety of \ac{ml} models in \ac{fhe} \cite{noauthor_concrete_2025}.
...
While concrete-ml and Zama are not the only ones trying to implement \ac{fhe} for \ac{ml} applications, from what we have seen concrete-ml seems to be by far the easiest, most complete, and most high-level solution at the time of writing.
Its main selling points are support for a large variety of models, including custom models, while requiring very little or no cryptography knowledge from the developer implementing these models.
This means that data scientists and in general people with some \ac{ml} experience but no \ac{fhe} or cryptography knowledge should be able to build \ac{fhe} models with concrete-ml without leaving the comfort of Python and Pytorch or scikit-learn.
...
Concrete largely implements this scheme as described in that section, with some extensions to increase its functionality and flexibility.
Its main innovation was \emph{\acf{pbs}} (which we already mentioned in \cref{TFHE_building_blocks}) which allows to execute functions that can be expressed as a look-up table during the Bootstrapping process.
...
However \ac{ml} models have to incorporate non-linear functions to be able to separate non-linear data which means that in order to run these models homomorphically we need to be able to evaluate these non-linear functions in \ac{fhe} as well.
...
Therefore we will always measure accuracy and F1 scores during our experiments to see how big of an impact the quantization has on any given models accuracy on each dataset.
...
The second one contains cryptographic parameters and descriptions of all required pre-/post-processing that needs to be done by the client (but reveals no information about the model or its weights).
...
We trained it using the LogisticRegression model from scikit-learn, and then used the \texttt{from\_sklearn\_model} method of the concrete-ml object to turn the trained model into its \ac{fhe} equivalent, using a relatively high bit-width of 8.
...
For this reason we expect the runtime overheads to be relatively low compared to the other models, and the accuracy loss due to quantization almost non-existent.
...
\subsubsection{XGB Tree-based Classifier}\label{XGBExp}
...
There are libraries like Microsoft's Hummingbird that aim to do exactly this in order to be able to accelerate these more classical \ac{ml} models with tensor operations on GPUs \cite{noauthor_microsofthummingbird_2025}.
...
The NeuralNetClassifier class implements a \ac{fnn} mimicking the scikit-learn class (as described in \cref{FNN}) and interface with the same name while in reality it is implemented with PyTorch.
...
\subsection{Encrypted Training with Built-In Model}\label{encrypted_training}
...
This might be interesting in fields where training data is very sparse and the only available data is confidential and cannot be shared with the entity performing the model training or \ac{ml} service.
...
\subsubsection{Custom PyTorch model with Embedding layer}\label{custom_pytorch_model}
...
Therefore the regularization also includes converting all characters lowercase.
...
Since we converted all tokens to lowercase, and since we the model only receives token indices and not the actual tokens, we added two auxiliary features to give the model some attributes of each of the tokens in addition to their indices: Capitalization information and token lengths.
...
Our PyTorch models full code can be seen in \cref{code:final_model}, but the general architecture looks like this:
...
We intentionally pushed the bit-width for this model to its limit (the fhe compilation would fail for anything higher than 5) since pushing the boundaries of \ac{fhe} was our goal here.
Therefore we expect even higher runtimes then for our \ac{fnn} experiment (see \cref{neural_net_exp}) while suffering from less performance degradation.
...
\subsubsection{Transformer Embeddings with \acs{knn} Classifier}\label{ner_knn-approach}
...
This might be challenging for a \ac{knn} search on its own, and even more so in an \ac{fhe} environment.
Due to this we added a dimensionality reduction with \ac{pca} so that we could freely change the embedding vectors dimension.
There is however a limit how far the vectors dimensionality can be reduced while still keeping its functionality as a semantic embedding intact.

\section{Datasets}\label{datasets}

We used a variation of datasets across different classification problems commonly solved with a \ac{ml} approach.
Some of these datasets are synthetic for maximum control over their characteristics to specifically measure a models behavior under certain conditions, while others are real-world datasets that we used to measure how well the models perform solving a variety of real-world problems, mimicking a production deployment of a homomorphic \ac{ml} setup as much as possible.
...
The datasets feature vectors are 2-dimensional.
...
Its small amount of features and samples make it great for quick testing and evaluation of \ac{ml} models.
...
This dataset is about telling the type of breast cancer by supplying a \ac{ml} model with a set of 30 properties of a fine needle aspirate of a patients breast mass (e.g. radius, texture, perimeter, etc.).
All of the aspirate properties in the dataset are from breasts affected by cancer, however some are affected by malignant cancer (label '0'), and some by benign cancer (label '1').
...
Second, the features (especially the ones for malignant cancer) spread quite heavily and much more than in our other datasets, with a maximum feature spread of $4030.4$.
...
For each variation, only 10\% of the features are actually informative of the samples label, while the other 90\% are just generated at random independently from the true labels.
...
Please note that by doing this, the visual representation of the dataset isn't accurate, for example in the plots it does not appear to be separable even though in reality it is.
...
Since our \ac{ner} approaches look at one token (and its context) at a time, we had to decrease the size of the dataset and balance it a bit.
