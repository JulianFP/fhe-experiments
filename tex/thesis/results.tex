...
We executed all our experiments $10$ times and calculated the average as well as the standard deviation between these executions.
Note that the error bars in all plots represent this standard deviation.

We also performed these experiments on two different machines to get a more complete picture of \ac{fhe} runtime behavior:
\begin{itemize}
    \item Our working groups compute server with 2x Intel Xeon Gold 6230R CPUs (in total 52 cores/104 threads @2.1GHz, up to 4.0GHz) with 192GB of memory, representing a machine with a high core count but a lower per core performance
    \item A personal computer with 1x AMD Ryzen 5 7600 CPU (6 cores/12 threads @3.8GHz, up to 5.1GHz) with 32GB of memory, representing a machine with a low core count but a higher per core performance
\end{itemize}

A lot of \ac{fhe} operations in concrete are very well parallelized while our clear executions of the same models are not.
Consequently we wanted to obtain runtime results from two separate configurations.
The PC environment is fairer towards the comparison between \ac{fhe} and clear executions since the \ac{fhe} execution cannot gain that much of a performance benefit by parallelization on a CPU with a small core count.
On the other hand the server environment represents a more realistic deployment for \ac{fhe} applications since enterprise servers and cloud machines tend to have high core counts but a lower per core performance.
We also want to point out that the server was being used by other people as well during the experiments, introducing a higher runtime variance between the executions.
The personal computer on the other hand was a more controlled environment with no other users and as little processes as possible running concurrently.
Due to this we expect the measured standard deviation to be much lower on the PC runs.

\section{Encrypted Inference with Built-In Models}

These are the results from our first group of experiments, described in \cref{encrypted_inference}.
Note that because of time constraints we did not execute the Neural Network experiment (see \cref{neural_net_exp}) on all variations of the 'SMS Spam' dataset, namely only up to a feature size of $100$.

\subsection{Accuracy and F1-Score}

\Cref{fig:server_acc_f1} provides an overview of the accuracy and F1-Scores of our three models evaluated on the server.
The comprehensive data these graphs are generated from can be found in \cref{table:inference_server_acc-f1} (server run) and \cref{table:inference_PC_acc-f1} (PC run).

\begin{figure}[h]
    \centering
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_acc_f1-plot_Logistic Regression.pdf}
        \end{center}
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_acc_f1-plot_XGB Tree-based Classification.pdf}
        \end{center}
        \caption{XGB Tree-based}
    \end{subfigure}
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_acc_f1-plot_Neural Network Classifier.pdf}
        \end{center}
        \caption{Neural Network}
    \end{subfigure}
    \caption{Accuracy and F1-Scores of all of our three models in both clear and FHE on various datasets, obtained from the server run of the experiments}\label{fig:server_acc_f1}
\end{figure}
...
For all datasets, the accuracy and F1-scores stayed exactly the same while also having a no standard deviation (since we fixed the seed for this experiment).
This proofs that with a high quantization bit-width and a very shallow \ac{fhe} inference circuit it is possible to completely avoid an accuracy loss of the \ac{fhe} model.

Our \ac{cart} model (see \cref{XGBExp}) only suffers from an extremely small accuracy reduction due to quantization which is not visible on the plot.
While this was better than expected, it is also worth noting that the \ac{pca} used in this experiment might have significantly contributed to this.
In fact, it might even have been much too aggressive since in theory any \ac{cart} model must be equal to or better than Logistic Regression.
Instead we observed varying improvements over Logistic Regression on some datasets ('XOR', most 'SMS Spam' variants), but a accuracy reductions on others.
As with most experiments, we implemented them as closely as possible to concrete-ml's documentations example code.
In this case however the recommended usage of \ac{pca} probably crippled the model quite substantially to the point were using it over Logistic Regression is not very sensible considering its increased runtimes.

For our \ac{fnn} experiments (see \cref{neural_net_exp}) we did not use a fixed seed since the concrete-ml's built-in model used here is not based on scikit-learn anymore and thus does not conveniently expose such functionality.
As a result our accuracies and F1-scores were subject to run-to-run variation which can be observed in the error bars.
The \ac{fnn} model has slightly better accuracies across the board, except for the 'SMS Spam' and 'Breast Cancer' datasets where it is slightly behind Logistic Regression.
The differences are so small however that they are mostly insignificant, hinting at the fact that our datasets are not challenging enough to expose the capability differences of the tested models.
Nevertheless we still found the \ac{fnn} to be the only model with acceptable accuracy on the 'XOR' dataset.
As expected the accuracy drops noticeable after compiling the model to \ac{fhe}, to quite varying degrees depending on the dataset.
While the 'Digits' and 'SMS Spam' datasets where barely affected, in comparison to the clear model the \ac{fhe} model performed much poorer on the 'Breast Cancer' and 'XOR' datasets which are very challenging for the quantization.

Except for smaller variations within the margin of error, the PC run yielded the same accuracy and F1-scores.

\subsection{Runtime}

The full runtime data for built-in model inference is available in \cref{table:inference_server_dur} (server run) and \cref{table:inference_PC_dur} (PC run).

\begin{figure}[h]
    \centering
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot_Logistic Regression.pdf}
        \end{center}
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot-with-ratio_Logistic Regression.pdf}
        \end{center}
        \caption{The FHE runtimes relative to the clear runtimes on a linear scale}
    \end{subfigure}
    \caption{Runtimes of the Logistic Regression model on various datasets, obtained from the server run of the experiments}\label{fig:server_runtime_logReg}
\end{figure}

The runtime increases due to \ac{fhe} of our Logistic Regression experiment (see \cref{fig:server_runtime_logReg}) started out to be relatively modest on datasets with smaller feature sizes with slowdowns around $100\times$.
However even this kind of slowdown means in practice that an extremely trivial operation like a vector product and addition (i.e. inference of a Logistic Regression model) that for a whole test dataset usually would take a single- to low two-digit amount of milliseconds becomes an operation taking a good fraction of a second or more.
...
The PC run (see \cref{fig:PC_runtime_logReg}) was faster for the clear execution than on the server, while the \ac{fhe} execution was slower, both by a significant margin.
This increases the slowdown even more, suggesting that while the clear model benefits from the stronger single-core performance of the PC, \ac{fhe} is much better parallelized and thus can benefit much more of the high core count of the server.
This is even more apparent for the \ac{fhe} pre-processing.
While post-processing benefits from higher single-core performance and experiences a significant performance improvement on the PC, pre-processing slows down so much more that in total our relative pre+post numbers increase by quite a lot compared to the server run.
...
This means that specifically for the pre+post processing results, the PC run is much more representative for real-world conditions than the server run.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot_XGB Tree-based Classification.pdf}
        \end{center}
        \caption{Both FHE and clear runtimes in absolute numbers on a logarithmic scale}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot-with-ratio_XGB Tree-based Classification.pdf}
        \end{center}
        \caption{The FHE runtimes relative to the clear runtimes on a linear scale}
    \end{subfigure}
    \caption{Runtimes of the XGB tree-based model on various datasets, obtained from the server run of the experiments}\label{fig:server_runtime_xgb}
\end{figure}

The \ac{cart} models shows a bit of a different runtime behavior (see \cref{fig:server_runtime_xgb}).
...
\begin{figure}[h]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot_Neural Network Classifier.pdf}
        \end{center}
        \caption{Both FHE and clear runtimes in absolute numbers on a logarithmic scale}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot-with-ratio_Neural Network Classifier.pdf}
        \end{center}
        \caption{The FHE runtimes relative to the clear runtimes on a linear scale}
    \end{subfigure}
    \caption{Runtimes of the Neural Network model on various datasets, obtained from the server run of the experiments}\label{fig:server_runtime_neural_net}
\end{figure}

For our \ac{fnn} model, we make another jump by an order of magnitude regarding relative slowdown, now being in the $\approx 10,000\times$ range of slowdown and even hitting more than $100,000\times$ on the 100 dimensions variation of Spam (see \cref{fig:server_runtime_neural_net}).
...
Since we executed our Synthetic and Spam datasets with a variety of feature sizes, we can look at how runtimes grow with the dimensionality of the input features (see \cref{fig:server_synth_feature-runtime}).

\begin{figure}[h]
    \centering
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_feature-runtime-plot_Logistic Regression_synth_.pdf}
        \end{center}
        \caption{Logistic Regression}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_feature-runtime-plot_XGB Tree-based Classification_synth_.pdf}
        \end{center}
        \caption{XGB Tree-based}
    \end{subfigure}
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_feature-runtime-plot_Neural Network Classifier_synth_.pdf}
        \end{center}
        \caption{Neural Network}
    \end{subfigure}
    \caption{Runtimes plotted against feature size of the 'Synthetic' dataset of all of our three models in both clear and FHE, plotted on a logarithmic scale, obtained from the server run of the experiments}\label{fig:server_synth_feature-runtime}
\end{figure}
...
These findings also translate to the SMS dataset, as well as the plots for both datasets of the PC run (see \cref{fig:server_spam_feature-runtime}, \cref{fig:PC_synth_feature-runtime}, and \cref{fig:PC_spam_feature-runtime} respectively).
...
As illustrated in \cref{fig:dec_bound_log-reg_iris}, for Logistic Regression the changes in the decision boundary mostly manifest in the boundary being a bit more granular due to quantization.
There are however exceptions to this rule, which can be observed for example on the 'Breast Cancer' dataset which has quite large feature dimensions and thus also shows very high changes in boundary granularity (see \cref{fig:dec_bound_log-reg_cancer}).
...
These are the results from our second group of experiments, described in \cref{encrypted_training}.
...
For the same reason we also only executed this experiment on the server and not on the PC.
...
\Cref{fig:server_acc_f1_training} presents the accuracy and F1-Scores of the model that resulted from encrypted \ac{sgd} training compared to a model that was trained with \ac{sgd} on clear data.
The detailed data is available in \cref{table:training_server_acc-f1}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{../figures/server_acc_f1-plot_SGD Classifier Training.pdf}
    \caption{Accuracy and F1-Scores of all of encrypted \ac{sgd} training in both clear and FHE on various datasets, obtained from the server run of the experiments}\label{fig:server_acc_f1_training}
\end{figure}
...
That being said, if we compare these values with our Logistic Regression model in \cref{fig:server_acc_f1} we can observe a clear accuracy reduction.
...
\Cref{fig:server_runtime_training} shows an overview of the runtimes of encrypted \ac{sgd} training.
The full data is available in \cref{table:training_server_dur}.

\begin{figure}[h]
    \centering
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot_SGD Classifier Training.pdf}
        \end{center}
        \caption{Both FHE and clear runtimes in absolute numbers on a logarithmic scale}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot-with-ratio_SGD Classifier Training.pdf}
        \end{center}
        \caption{The FHE runtimes relative to the clear runtimes on a linear scale}
    \end{subfigure}
    \caption{Runtimes of the encrypted \ac{sgd} training on various datasets, obtained from the server run of the experiments}\label{fig:server_runtime_training}
\end{figure}

Here we observed much longer runtimes for encrypted training than for the encrypted inference that we looked at before.
...
This is the largest slowdown that we have seen throughout this thesis, especially considering that this was executed on the server and not the PC.
...
The plots for this can be found in \cref{fig:server_training_synth_spam_feature-runtime}.

\section{Encrypted Inference with custom approaches for the \acl{ner} task}

These are the results from our third and final group of experiments, described in \cref{ner_exps}.
...
\Cref{fig:server_acc_f1_ner} shows an overview of the accuracy and F1-Scores of two \ac{ner} approaches from the server run. The full data is available in \cref{table:ner_server_acc-f1} (server run) and \cref{table:ner_PC_acc-f1} (PC run).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{../figures/server_acc_f1-plot-ner.pdf}
    \caption{Accuracy and F1-Scores of our two \ac{ner} approaches in both clear and FHE on the \ac{cconll} dataset, obtained from the server run of the experiments}\label{fig:server_acc_f1_ner}
\end{figure}
...
\Cref{fig:server_runtime_ner} shows an overview of the runtimes of our two \ac{ner} approaches from the server run. The full data is available in \cref{table:ner_server_dur} (server run) and \cref{table:ner_PC_dur} (PC run).

\begin{figure}[h]
    \centering
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot-ner.pdf}
        \end{center}
        \caption{Both FHE and clear runtimes in absolute numbers on a logarithmic scale}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\textwidth}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{../figures/server_runtime-plot-ner-with-ratio.pdf}
        \end{center}
        \caption{The FHE runtimes relative to the clear runtimes on a linear scale}
    \end{subfigure}
    \caption{Runtimes of our two \ac{ner} approaches in both clear and FHE on the \ac{cconll} dataset, obtained from the server run of the experiments}\label{fig:server_runtime_ner}
\end{figure}

The relative slowdown for the processing step of our custom PyTorch model was in the same order of magnitude as that of the Built-In \ac{fnn} model in \cref{fig:server_runtime_neural_net}.
...
This significant difference between server and PC execution shows how much \ac{fhe} relies on performance improvements coming from parallelization for these larger and complexer models.
