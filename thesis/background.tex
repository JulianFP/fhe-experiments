...
The \textbf{"Leveled schemes branch"}, which implement so called Leveled FHE. While these schemes also use bootstrapping, they can only evaluate circuits up to a pre-set depth d. This depth can be arbitrary, however it must be known at encryption time \cite{gentry_fully_2009}. This limitation only matters if in a specific application the circuit depth is significantly dependent on the input and thus dynamic and not known beforehand. For example Performing inference on a neural network would be an use case where this does not matter, since a forward through the model always goes through a fixed amount of layers and thus operations. These schemes are also not defined on bit operations, but on integer and (approximate) floating point operations.
...
\begin{equation}
    \begin{aligned}
        \langle s, a_1 \rangle + e_1 &= b_1 \quad (\Mod p)\\
        \langle s, a_2 \rangle + e_2 &= b_2 \quad (\Mod p)\\
        \langle s, a_3 \rangle + e_3 &= b_3 \quad (\Mod p)\\
        \vdots&
    \end{aligned}
\end{equation}
