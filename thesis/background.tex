...
In \ref{DGHV} we built a simple homomorphic encryption scheme based on the AGCD problem. While there are many mathematical problems that have been used to base HE schemes on, the most prominent and pretty much the only ones that don't have known security flaws against them are based in some way on the Learning With Errors (LWE) problem, or its ring version, the Ring-LWE problem \cite{smart_final_2022}.
