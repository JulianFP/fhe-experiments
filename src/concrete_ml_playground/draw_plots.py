...
def draw_decision_boundary(
    ...
    clear_title = f"{exp_name} - {dset_name} - clear"
    fhe_title = f"{exp_name} - {dset_name} - FHE"
    os.makedirs("results", exist_ok=True)

    clear_pickle_path = f"results/{clear_title}.pickle"
    if os.path.isfile(clear_pickle_path):
    ...
    if os.path.isfile(fhe_pickle_path):
        with open(fhe_pickle_path, "rb") as file:
            Z_fhe = pickle.load(file)
    elif plot_data.fhe_trained_model is not None:
